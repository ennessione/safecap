...
public class ConstantFolding extends CLGenericRewriter {
	private SDARuntimeExecutionContext model;

	public ConstantFolding(SDARuntimeExecutionContext model) {
		this.model = model;
	}
		...
		if (original.getTag() == alphabet.OP_UNION || original.getTag() == alphabet.OP_INTER) {
			...
			for (CLExpression p : args) {
				if (CLUtils.isConstant(p, model)) {
					if (constVal == null)
						constVal = (BSet) p.getValue(model);
					else if (original.getTag() == alphabet.OP_UNION)
						constVal = constVal._union((BSet) p.getValue(model));
					else if (original.getTag() == alphabet.OP_INTER)
						constVal = constVal._inter((BSet) p.getValue(model));
				} else {
					newArgs.add(p);
				}
			}
	...
	private CLExpression getValue(CLExpression original) {
		try {
			Object value = original.getValue(model);
			if (value == null)
				return null;
