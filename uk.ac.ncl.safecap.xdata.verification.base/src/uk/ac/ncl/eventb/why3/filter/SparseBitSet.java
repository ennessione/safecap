 ...
 * <code>SparseBitSet</code> are indexed by nonnegative integers. Individual
 ...
 * subsequent bit (<i>getNextSetBit</i>()), will return an value of &minus;1. If
 * an attempt is made to use <i>getNextClearBit</i>(), and all the bits are set
 * from the starting postion of the search to the bit labelled
...
public class SparseBitSet implements Cloneable, Serializable {
	/*
	 * My apologies for listing all the additional authors, but concepts, code, and
	 * even comments have been re-used in this class definition from code in the JDK
	 * that was written and/or maintained by these people. I owe a debt, which I
	 * acknowledge. But they are in no way responsible for what ever misuse I have
	 * made of their work. Bruce K. Haddon
	 * 
	 ...
	 * accessed by two levels of indirection from the master "level 1" (whole) set
	 * array through second level arrays called "areas" (making the blocks
	 * "level3"). A "word" is a long, consisting of 64 bits, requiring six address
	 * bits to select a bit within a word (and this is considered in places to be a
	 * "level4"). This choice of a long for "word" is determined purely by
	 * performance concerns, and is built into the implementation in a deep way,
	 * because the references to blocks are always of the form "long[]." (This does
	 * not mean that blocks could not be changed to arrays of ints, but it would
	 * take some extensive work.)
	 * 
	 * The fact that there are three levels is also deeply involved in the scanning
	 * algorithms, meaning that the accesses are always nested three deep. Again,
	 * the change this might be a large amount of work. On the other hand, these
	 * three levels have proven, so far, to provide adequate speed, and an storage
	 * efficient way to deal with sparseness.
	 * 
	 * For simplicity, the level3 blocks and the level2 areas are always "full"
	 * size, i.e., LENGTH3 and LENGTH2 respectively, and for consistency, the fourth
	 * level is of length LENGTH4. The level1 structure is of variable length (as
	 * this may save scanning several thousand null pointers, and careful
	 * consideration must be taken of this at all times, in particular, when
	 * choosing to increase the size (see resize()). The only place where this array
	 * is reduced in size is when a clone is made, in which case it is created with
	 * the smallest size, and allowed to grow as entries are copied to it. That all
	 * the arrays are kept to power-of-2 sizes is a programming convenience
	 * (permitting shifts and masks).
	 * 
	 * Whenever possible, a level 3 block that contains no bits (all the words are
	 * zero, is discarded, and its reference is replaced by a null value. Similarly,
	 * level 2 areas that contain only null pointers (to level 3 blocks) are
	 * discarded, and their references replaced by null values. This is the
	 * "normalized" condition, but does not have to be nor is strictly enforced. The
	 * operations still work if the representation is partially or totally
	 * "denormalized." In particular, the methods that deal with single bits
	 * (setting, flipping, clearing, etc.) do not attempt to normalize the set, in
	 * the interests of speed. However, when a set is scanned as the resultant set
	 * of some operation, then, in most cases, the set will be normalized--the
	 * exception being level2 areas that are not completly scanned in a particular
	 * pass.
	 * 
	 ...
	 * "sweet" spot. There is, of course, no guarantee that these are the best for
	 * all possible situations, but, given that not having these the same for all
	 * bit sets would be hopelessly complex (bad enough as is), these values appear
	...
	/**
	 * For the current size of the bits array, this is the maximum possible length
	 * of the bit set, i.e., the index of the last possible bit, plus one. Note:
	 * this not the value returned by <i>length</i>().
	 * 
	 * @see #resize(int)
	 * @see #length()
	 */
	...
	/**
	 * The number of bits in a positive integer, and the size of permitted index of
	 * a bit in the bit set.
	 */
	protected static final int INDEX_SIZE = Integer.SIZE - 1;

	/**
	 * The label (index) of a bit in the bit set is essentially broken into 4
	 * "levels". Respectively (from the least significant end), level4, the address
	 * within word, the address within a level3 block, the address within a level2
	 * area, and the level1 address of that area within the set.
	 *
	 * LEVEL4 is the number of bits of the level4 address (number of bits need to
	 * address the bits in a long)
	 */
	...
	/**
	 * SHIFT2 is the shift to bring the level2 address (from the word index) to the
	 * right end (i.e., after shifting by SHIFT3).
	 */
	...
	/**
	 * SHIFT1 is the shift to bring the level1 address (from the word index) to the
	 * right end (i.e., after shifting by SHIFT3).
	 */
	...
	/**
	 * A spare level 3 block is kept for use when scanning. When a target block is
	 * needed, and there is not already one in the bit set, the spare is provided.
	 * If non-zero values are placed into this block, it is moved to the resulting
	 * set, and a new spare is acquired. Note: a new spare needs to be allocated
	 * when the set is cloned (so that the spare is not shared between two sets).
	 */
	protected transient long[] spare;

	/**
	 * An empty level 3 block is kept for use when scanning. When a source block is
	 * needed, and there is not already one in the corresponding bit set, the
	 * ZERO_BLOCK is used (as a read-only block). It is a source of zero values so
	 * that code does not have to test for a null level3 block. This is a static
	 * block shared everywhere.
	 */
	 ...
	 * Programming notes:
	 * 
	 * i, j, and k are used to hold values that are actual bit indices (i.e., the
	 * index (label) of the bit within the user's view of the bit set).
	 * 
	 * u, v, and w, are used to hold values that refer to the indices of the words
	 * in the set array that are used to hold the bits (with 64 bits per word).
	 * These variable names, followed by 1, 2, or 3, refer to the component "level"
	 * parts of the complete word index.
	 * 
	 * word (where used) is a potential entry to or from a block, containing 64 bits
	 * of the bit set. The prefixes a, b, result, etc., refer to the bit sets from
	 * which these are coming or going. Without a prefix, or with the prefix "a,"
	 * the set in question is "this" set (see next paragraph).
	 * 
	 * Operations are conceived to be in the form a.op(b), thus in the discussion
	 * (not in the public Javadoc documentation) the two sets are referred to a "a"
	 * and "b", where the set referred to by "this" is usually set a. Hence,
	 * reference to set a is usually implicit, but set b will usually be explicit.
	 * Variables beginning with these letters hold values relevant to the
	 * corresponding set, and, in particular, these letters followed by 1, 2, and 3
	 * are used to refer to the corresponding (current) level1, level3 area, and
	 * level3 block, arrays.
	 * 
	 ...
	 * 
	 * Similarly, care it taken to ensure that any supplied reference to a bit set
	 * (other than this) has an opportunity to fail for being null before any other
	 * set (including this) has its state changed. For the most part, this is
	 * allowed to happen "naturally," but the Strategies incorporate an explicit
	 * check when necessary.
	 * 
	 * There is a amount of (almost) repetitive scanning code in many of the
	 * "singe bit" methods. The intent is that these methods for SparseBitSet be as
	 * small and as fast as possible.
	 * 
	 * For the scanning of complete sets, or for ranges within complete sets, all of
	 * the scanning logic is built into one (somewhat enormous) method,
	 * setScanner(). This contains all the considerations for matching up
	 * corresponding level 3 blocks (if they exist), and then uses a Strategy object
	 * to do the processing on those level3 blocks. This keeps all the scanning and
	 * optimization logic in one place, and the Strategies are reasonably simple
	 * (see the definition of AbstractStrategy for a discussion of the tasks that
	 * must be defined therein).
	 * 
	 * The test for index i (the first index in all cases) being in range is rather
	 * perverse, but the idea was to keep the actual number of comparisons to a
	 * minimum, hence the check is for "(i + 1) < 1". This is almost but not quite
	 * equivalent to "i < 0", although it is for all values of i except
	 ...
	 * check picks up the other disallowed case. Let us hope the compiler never gets
	 * smart enough to try to do the apparent optimisation!
	 */
	 ...
	 * <p>
	 * The capacity is taken to be a <i>suggestion</i> for a size of the bit set, in
	 * bits. An appropiate table size (a power of two) is then determined and used.
	 * The size will be grown as needed to accomodate any bits addressed during the
	 * use of the bit set.
	 *
	 * @param capacity        a size in terms of bits
	 * @param compactionCount the compactionCount to be inherited (for internal
	 *                        generation)
	...
	/**
	 * Constructs an empty bit set with the default initial size. Initially all bits
	 * are effectively <code>false</code>.
	 *
	 * @since 1.6
	 */
	public SparseBitSet() {
	...
	/**
	 * Creates a bit set whose initial size is large enough to efficiently represent
	 * bits with indices in the range <code>0</code> through at least
	 * <code>nbits-1</code>. Initially all bits are effectively <code>false</code>.
	 * <p>
	 * No guarantees are given for how large or small the actual object will be. The
	 * setting of bits above the given range is permitted (and will perhaps
	 * eventually cause resizing).
	 *
	 * @param nbits the initial provisional length of the SparseBitSet
	 * @throws java.lang.NegativeArraySizeException if the specified initial length
	 *         is negative
	 * @see #SparseBitSet()
	 * @since 1.6
	 */
	 ...
	 * @exception IndexOutOfBoundsException if the specified index is negative or
	 ...
	 * bit in the bit set argument also had the value <code>true</code>. Outside the
	 ...
	 * @exception IndexOutOfBoundsException if <code>i</code> is negative or equal
	 *                                      to Integer.MAX_VALUE, or <code>j</code>
	 *                                      is negative, or <code>i</code> is larger
	 *                                      than <code>j</code>
	 * @since 1.6
	 */
	 ...
	 * <code>true</code> if and only if it both initially had the value
	 * <code>true</code> and the corresponding bit in the bit set argument also had
	 ...
	 * The returned <code>SparseBitSet</code> is created so that each bit in it has
	 * the value <code>true</code> if and only if both the given sets initially had
	 * the corresponding bits <code>true</code>, otherwise <code>false</code>.
	 *
	...
	/**
	 * Performs a logical <b>AndNOT</b> of this target bit set with the argument bit
	 * set within the given range of bits. Within the range, this bit set is
	 * modified so that each bit in it has the value <code>true</code> if and only
	 * if it both initially had the value <code>true</code> and the corresponding
	 * bit in the bit set argument has the value <code>false</code>. Outside the
	 * range, this set is not changed.
	 *
	 * @param i index of the first bit to be included in the operation
	 * @param j index after the last bit to included in the operation
	 ...
	 * set. This bit set is modified so that each bit in it has the value
	 * <code>true</code> if and only if it both initially had the value
	 * <code>true</code> and the corresponding bit in the bit set argument has the
	 ...
	 * <code>SparseBitSet</code>. The resulting bit set is created so that a bit in
	 * it has the value <code>true</code> if and only if the corresponding bit in
	 * the <code>SparseBitSet</code> of the first is set, and that same
	...
	public void clear(int i) {
		/*
		 * In the interests of speed, no check is made here on whether the level3 block
		 * goes to all zero. This may be found and corrected in some later operation.
		 */
		if (i + 1 < 1) {
			throw new IndexOutOfBoundsException("i=" + i);
		}
	 ...
	 * <code>j</code> (exclusive) to <code>false</code>.
	 *
	...
	/**
	 * Sets all of the bits in this <code>SparseBitSet</code> to <code>false</code>.
	 *
	 * @since 1.6
	 */
	public void clear() {
	 ...
	 * <code>SparseBitSet</code> that is <i>equal</i>() to it. The clone of the bit
	 ...
	 * amount of storage allocated to hold the bits, while still trying to keep
	 * access to the bits being a rapid as possible. Since the space allocated to a
	 * <code>SparseBitSet</code> is not normally decreased, replacing a bit set by
	 * its clone may be a way of both managing memory consumption and improving the
	 * rapidity of access.
	 *
	...
	public SparseBitSet clone() {
		try {
			...
			/*
			 * Clear out the shallow copy of the set array (which contains just copies of
			 * the references from this set), and then replace these by a deep copy (created
			 * by a "copy" from the set being cloned .
			 */
			result.bits = null;
			result.resize(1);
			/*
			 * Ensure the clone is not sharing a copy of a spare block with the cloned set,
			 * nor the cache set, nor any of the visitors (which are linked to their parent
			 * object) (Not all visitors actually use this link to their containing object,
			 * but they are reset here just in case of future changes).
			 */
			...
			/*
			 * This code has not been unit tested. Inspection offers hope that is will work,
			 * but it likely never to be used.
			 */
	 ...
	 * a <code>SparseBitSet</code> object that has exactly the same bits set to
	 * <code>true</code> as this bit set. That is, for every nonnegative
	 ...
	 * @return <code>true</code> if the objects are equivalent; <code>false</code>
	 *         otherwise.
	 * @since 1.6
	 */
	@Override
	...
	/**
	 * Sets each bit from the specified <code>i</code> (inclusive) to the specified
	 * <code>j</code> (exclusive) to the complement of its current value.
	 *
	...
	public boolean get(int i) {
		...
		long[][] a2;
		long[] a3;
		return i < bitsLength && (a2 = bits[w >> SHIFT1]) != null && (a3 = a2[w >> SHIFT2 & MASK2]) != null
				&& (a3[w & MASK3] & 1L << i) != 0;
	}
	 ...
	 * which bits have been set within this <code>SparseBitSet</code>. The algorithm
	 * used to compute it may be described as follows.
	 * <p>
	 * Suppose the bits in the <code>SparseBitSet</code> were to be stored in an
	 * array of <code>long</code> integers called, say, <code>bits</code>, in such a
	 * manner that bit <code>i</code> is set in the <code>SparseBitSet</code> (for
	 * nonnegative values of <code>i</code>) if and only if the expression
	 * 
	 * <pre>
	...
	/**
	 * Returns true if the specified <code>SparseBitSet</code> has any bits within
	 * the given range <code>i</code> (inclusive) to <code>j</code> (exclusive) set
	 * to <code>true</code> that are also set to <code>true</code> in the same range
	 * of this <code>SparseBitSet</code>.
	 *
	 * @param i index of the first bit to include
	 * @param j index after the last bit to include
	 ...
	 * @return boolean indicating whether this SparseBitSet intersects the specified
	 *         SparseBitSet
	 * @since 1.6
	 */
	...
	/**
	 * Returns true if this <code>SparseBitSet</code> contains no bits that are set
	 * to <code>true</code>.
	 *
	...
	/**
	 * Returns the "logical length" of this <code>SparseBitSet</code>: the index of
	 * the highest set bit in the <code>SparseBitSet</code> plus one. Returns zero
	 * if the <code>SparseBitSet</code> contains no set bits.
	 *
	...
	public int nextClearBit(int i) {
		/*
		 * The index of this method is permitted to be Integer.MAX_VALUE, as this is
		 * needed to make this method work together with the method nextSetBit()--as
		 * might happen if a search for the next clear bit is started after finding a
		 * set bit labelled Integer.MAX_VALUE-1. This case is not optimised, the code
		 * will eventually return -1 (since the Integer.MAX_VALUEth bit does "exist,"
		 * and is 0.
		 */
		 ...
		 * (including the nominated beginning bit itself). The first check is whether
		 * the starting bit is within the structure at all.
		 */
		if (w1 < aLength && (a2 = bits[w1]) != null && (a3 = a2[w2]) != null && (nword = ~a3[w3] & ~0L << i) == 0L) {
			/*
			 * So now start a search though the rest of the entries for a null area or
			 * block, or a clear bit (a set bit in the complemented value).
			 */
			++w;
			w3 = w & MASK3;
			w2 = w >> SHIFT2 & MASK2;
	 ...
	 * occurs on or after the specified starting index. If no such it exists then -1
	 * is returned.
	 * <p>
	...
	public int nextSetBit(int i) {
		/*
		 * The index value (i) of this method is permitted to be Integer.MAX_VALUE, as
		 * this is needed to make the loop defined above work: just in case the bit
		 * labelled Integer.MAX_VALUE-1 is set. This case is not optimised: but
		 * eventually -1 will be returned, as this will be included with any search that
		 * goes off the end of the level1 array.
		 */
	...
	/**
	 * Performs a logical <b>OR</b> of the addressed target bit with the argument
	 * value. This bit set is modified so that the addressed bit has the value
	 * <code>true</code> if and only if it both initially had the value
	...
	/**
	 * Performs a logical <b>OR</b> of the addressed target bit with the argument
	 * value within the given range. This bit set is modified so that within the
	 * range a bit in it has the value <code>true</code> if and only if it either
	 * already had the value <code>true</code> or the corresponding bit in the bit
	 * set argument has the value <code>true</code>. Outside the range this set is
	 * not changed.
	 *
	 * @param i index of the first bit to be included in the operation
	 * @param j index after the last bit to included in the operation
	 * @param b the SparseBitSet with which to perform the <b>OR</b> operation with
	 *          this SparseBitSet
	 * @exception IndexOutOfBoundsException if <code>i</code> is negative or equal
	 *                                      to Integer.MAX_VALUE, or <code>j</code>
	 *                                      is negative, or <code>i</code> is larger
	 ...
	 * bit set is modified so that a bit in it has the value <code>true</code> if
	 * and only if it either already had the value <code>true</code> or the
	...
	/**
	 * Performs a logical <b>OR</b> of the two given <code>SparseBitSet</code>s. The
	 * returned <code>SparseBitSet</code> is created so that a bit in it has the
	 * value <code>true</code> if and only if it either had the value
	 ...
	 * <code>SparseBitSet</code> to represent bit values. The count of bits in the
	 * set is the (label of the last set bit) + 1 - (the label of the first set
	 * bit).
	 *
	 * @return the number of bits (true and false) nominally in this bit set at this
	 *         moment
	 * @since 1.6
	 */
	public int size() {
	 ...
	 * Level3 blocks in use,, Level2 pool size, Level3 pool size, and the Compaction
	 * count.
	 * <p>
	 * This method is intended for diagnostic use (as it is relatively expensive in
	 * time), but can be useful in understanding an application's use of a
	 * <code>SparseBitSet</code>.
	 *
	...
	public String statistics(String[] values) {
		...
		final String[] v = new String[Statistics.values().length];

		/*
		 * Assign the statistics values to the appropriate entry. The order of the
		 * assignments does not matter--the ordinal serves to get the values into the
		 * matching order with the labels from the enumeration.
		 */
		...
		int longestLabel = 0;
		for (final Statistics s : Statistics.values()) {
		 ...
		 * "Average_length_value", and "Average_chain_length" are printed as floating
		 * point values.
		 */
	 ...
	 * listed in order from lowest to highest. If there is a subsequence of set bits
	 * longer than the value given by toStringCompaction, the subsequence is
	 * represented by the value for the first and the last values, with ".." between
	 * them. The individual bits, or the representation of sub-sequences are
	 ...
	 * <p>
	 * This method is intended for diagnostic use (as it is relatively expensive in
	 * time), but can be useful in interpreting problems in an application's use of
	 * a <code>SparseBitSet</code>.
	 *
	...
	public String toString() {
		...
		while (i >= 0) {
			...
			if (compactionCount > 0) {
				...
				/*
				 * Compute the position of the next clear bit after the current subsequence of
				 * set bits.
				 */
				last = last < 0 ? Integer.MAX_VALUE : last;
				/*
				 * If the subsequence is more than the specified bits long, then collapse the
				 * subsequence into one entry in the String.
				 */
				if (i + compactionCount < last) {
					p.append("..").append(last - 1);
					/*
					 * Having accounted for a subsequence of bits that are all set, recompute the
					 * label of the next bit to show.
					 */
					j = nextSetBit(last);
				}
			}
			/*
			 * If there is another set bit, put a comma and a space after the last entry in
			 * the String.
			 */
			if (j >= 0) {
				p.append(", ");
	...
	/**
	 * Sequences of set bits longer than this value are shown by {@link #toString()}
	 * as a "sub-sequence," in the form <code>a..b</code>. Setting this value to
	 * zero causes each set bit to be listed individually. The default default value
	 * is 2 (which means sequences of three or more bits set are shown as a
	 * subsequence, and all other set bits are listed individually).
	 * <p>
	 * Note: this value will be passed to <code>SparseBitSet</code>s that may be
	 * created within or as a result of the operations on this bit set, or, for
	 * static methods, from the value belonging to the first parameter.
	 *
	 * @param count the maximum count of a run of bits that are shown as individual
	 *              entries in a </code>toString</code>() conversion. If 0, all bits
	...
	public void toStringCompaction(boolean change) {
		/*
		 * This is an assignment to a static value: the integer value assignement is
		 * atomic, so there will not be a partial store. If multiple invocations are
		 * made from multiple threads, there is a race condition that cannot be resolved
		 * by synchronization.
		 */
		if (change) {
	...
	/**
	 * Performs a logical <b>XOR</b> of the addressed target bit with the argument
	 * value. This bit set is modified so that the addressed bit has the value
	 * <code>true</code> if and only one of the following statements holds:
	 * <ul>
	 * <li>The addressed bit initially had the value <code>true</code>, and the
	 * value of the argument is <code>false</code>.
	 * <li>The bit initially had the value <code>false</code>, and the value of the
	 ...
	 *            negative or equal to Integer.MAX_VALUE
	 * @since 1.6
	 */
	 ...
	 * within the range in it has the value <code>true</code> if and only if one of
	 * the following statements holds:
	 * <ul>
	 * <li>The bit initially had the value <code>true</code>, and the corresponding
	 * bit in the argument set has the value <code>false</code>.
	 * <li>The bit initially had the value <code>false</code>, and the corresponding
	 * bit in the argument set has the value <code>true</code>.
	 * </ul>
	 * Outside the range this set is not changed.
	 *
	 * @param i index of the first bit to be included in the operation
	 * @param j index after the last bit to included in the operation
	 * @param b the SparseBitSet with which to perform the <b>XOR</b> operation with
	 *          this SparseBitSet
	 * @exception IndexOutOfBoundsException if <code>i</code> is negative or equal
	 *                                      to Integer.MAX_VALUE, or <code>j</code>
	 *                                      is negative, or <code>i</code> is larger
	 ...
	 * resulting bit set is computed so that a bit in it has the value
	 ...
	 * @since 1.6
	 */
	protected static final void throwIndexOutOfBoundsException(int i, int j) throws IndexOutOfBoundsException {
		String s = "";
		if (i < 0) {
			s += "(i=" + i + ") < 0";
		}
	...
	/**
	 * Clear out a part of the set array with nulls, from the given start to the end
	 * of the array. If the given parameter is beyond the end of the bits array,
	 * nothing is changed.
	 *
	 ...
	 * SparseBitSet into an array whose size (which may be larger or smaller) is the
	 * given bit size (<i>i.e.</i>, includes the bit whose index is one less that
	 * the given value). If the new array is smaller, the excess entries in the set
	 * array are discarded. If the new array is bigger, it is filled with nulls.
	 *
	...
	protected final void resize(int index) {
		/*
		 * Find an array size that is a power of two that is as least as large enough to
		 * contain the index requested.
		 */
	 ...
	 * all performed by this method. The properties and the operation executed are
	 * defined by a given strategy, which must be derived from the
	 * <code>AbstractStrategy</code>. The strategy defines how to operate on a
	 * single word, and on whole words that may or may not constitute a full block
	 * of words.
	 *
	 * @param i  the bit (inclusive) at which to start the scan
	 * @param j  the bit (exclusive) at which to stop the scan
	 * @param b  a SparseBitSet, if needed, the second SparseBitSet in the operation
	 * @param op the AbstractStrategy class defining the operation to be executed
	...
	protected final void setScanner(int i, int j, SparseBitSet b, AbstractStrategy op) throws IndexOutOfBoundsException {
		/*
		 * This method has been assessed as having a McCabe cyclomatic complexity of 47
		 * (i.e., impossibly high). However, given that this method incorporates all the
		 * set scanning logic for all methods (with the exception of nextSetBit and
		 * nextClearBit, which themselves have high cyclomatic complexities of 13), and
		 * is attempting to minimise execution time (hence deals with processing
		 * shortcuts), it cannot be expected to be simple. In fact, the work of lining
		 * up level3 blocks proceeds step-wise, and each sub-section piece is reasonably
		 * straight-forward. Nevertheless, the number of paths is high, and caution is
		 * advised in attempting to correct anything.
		 */
		...
		/*
		 * Index of the current word, and mask for the first word, to be processed in
		 * the bit set.
		 */
		int u = i >> SHIFT3;
		final long um = ~0L << i;
		...
		/*
		 * Index of the final word, and mask for the final word, to be processed in the
		 * bit set.
		 */
		...
		/*
		 * Set up the two bit arrays (if the second exists), and their corresponding
		 * lengths (if any).
		 */
		...
		/*
		 * Calculate the initial values of the parts of the words addresses, as well as
		 * the location of the final block to be processed.
		 */
		int u1 = u >> SHIFT1;
		...
		/*
		 * Initialize the local copies of the counts of blocks and areas; and whether
		 * there is a partial first block.
		 */
		int a2CountLocal = 0;
		int a3CountLocal = 0;
		...
		/*
		 * The first level2 is cannot be judged empty if not being scanned from the
		 * beginning.
		 */
		boolean a2IsEmpty = u2 == 0; // Presumption
		while (i < j) {
			/*
			 * Determine if there is a level2 area in both the a and the b set, and if so,
			 * set the references to these areas.
			 */
			long[][] a2 = null;
			...
			/*
			 * Handling of level 2 empty areas: determined by the properties of the
			 * strategy. It is necessary to actually visit the first and last blocks of a
			 * scan, since not all of the block might participate in the operation, hence
			 * making decision based on just the references to the blocks could be wrong.
			 */
			if ((!haveA2 && !haveB2 && f_op_f_eq_f || !haveA2 && f_op_x_eq_f || !haveB2 && x_op_f_eq_f) && notFirstBlock && u1 != v1) {// nested
																																		// if!
				if (u1 < aLength1) {
					a1[u1] = null;
				}
				...
				while (u2 != limit2) {
					/*
					 * Similar logic applied here as for the level2 blocks. The initial and final
					 * block must be examined. In other cases, it may be possible to make a decision
					 * based on the value of the references, as indicated by the properties of the
					 * strategy.
					 */
					long[] a3 = null;
					...
					/*
					 * Handling of level 3 empty areas: determined by the properties of the
					 * strategy.
					 */
					if ((!haveA3 && !haveB3 && f_op_f_eq_f || !haveA3 && f_op_x_eq_f || !haveB3 && x_op_f_eq_f) && notFirstBlock
							&& notLastBlock) {
						...
						{// nested if!
							/*
							 * If there is an level 2 area make the entry for this level3 block be a null
							 * (i.e., remove any a3 block ).
							 */
							if (haveA2) {
								a2[u2] = null;
							}
							...
							/*
							 * If the a3 block used was the spare block, put it into current level2 area;
							 * get a new spare block.
							 */
							if (a3 == spare) {
				...
				/*
				 * If the loop finishes without completing the level 2, it may be left with a
				 * reference but still be all null--this is OK.
				 */
	...
	/**
	 * The entirety of the bit set is examined, and the various statistics of the
	 * bit set (size, length, cardinality, hashCode, etc.) are computed. Level
	 ...
	 *             <i>length</i> of the set (the position of the last bit), followed
	 *             by the <i>cache.count</i> value (an <code>int</code>, the number
	 *             of <code>int->long</code> pairs needed to describe the set),
	 *             followed by the index (<code>int</code>) and word
	 *             (<code>long</code>) for each <code>int->long</code> pair. The
	 *             mappings need not be emitted in any particular order. This is
	 *             followed by the <i>hashCode</i> for the set that can be used as
	 *             an integrity check when the bit set is read back.
	 *
	 * @since 1.6
	 */
	 ...
	 * <p>
	 * An array of <code>String</code>s may be obtained containing a representation
	 * of each of these values. An element of such an array, say,
	...
	// =============================================================================
	/**
	 * This class holds the values related to various statistics kept about the bit
	 * set. These values are not kept continuously up-to-date. Whenever the values
	 * become invalid, the field <i>hash</i> is set to zero, indicating that an
	 * update is required.
	 *
	...
	protected class Cache {
		/**
		 * <i>hash</i> is updated by the <i>statisticsUpdate</i>() method. If the
		 * <i>hash</i> value is zero, it is assumed that <b><i>all</i></b> the cached
		...
		/**
		 * <i>size</i> is updated by the <i>statisticsUpdate</i>() method. If the
		 * <i>hash</i> value is zero, it is assumed the all the cached values are stale,
		 * and must be updated.
		 */
		protected transient int size;

		/**
		 * <i>cardinality</i> is updated by the <i>statisticsUpdate</i>() method. If the
		 * <i>hash</i> value is zero, it is assumed the all the cached values are stale,
		 * and must be updated.
		 */
		...
		/**
		 * <i>length</i> is updated by the <i>statisticsUpdate</i>() method. If the
		 * <i>hash</i> value is zero, it is assumed the all the cached values are stale,
		 * and must be updated.
		 */
		...
		/**
		 * <i>count</i> is updated by the <i>statisticsUpdate</i>() method. If the
		 * <i>hash</i> value is zero, it is assumed the all the cached values are stale,
		 * and must be updated.
		 */
		...
		/**
		 * <i>a2Count</i> is updated by the <i>statisticsUpdate</i>() method, and will
		 * only be correct immediately after a full update. The <i>hash</i> value is
		 * must be zero for all values to be updated.
		 */
		protected transient int a2Count;

		/**
		 * <i>a3Count</i> is updated by the <i>statisticsUpdate</i>() method, and will
		 * only be correct immediately after a full update. The <i>hash</i> value is
		 * must be zero for all values to be updated.
		 */
	...
	// =============================================================================
	/**
	 * This strategy class is used by the setScanner to carry out the a variety of
	 * operations on this set, and usually a second set. The <i>setScanner</i>()
	 * method of the main <code>SparseBitSet</code> class essentially finds matching
	 * level3 blocks, and then calls the strategy to do the appropriate operation on
	 * each of the elements of the block.
	 * <p>
	 * The symbolic constants control optimisation paths in the <i>setScanner</i>()
	 * method of the main <code>SparseBitSet</code> class.
	 *
	 * @see SparseBitSet#setScanner(int i, int j, SparseBitSet b, AbstractStrategy
	 *      op)
	 */
	protected abstract class AbstractStrategy {
		/**
		 * If the operation requires that when matching level2 areas or level3 blocks
		 * are null, that no action is required, then this property is required.
		 * Corresponds to the top-left entry in the logic diagram for the operation
		 * being 0. For all the defined actual logic operations ('and', 'andNot', 'or',
		 * and 'xor', this will be true, because for all these, "false" op "false" =
		 * "false".
		 */
		static final int F_OP_F_EQ_F = 0x1;

		/**
		 * If when level2 areas or level3 areas from the this set are null will require
		 * that area or block to remain null, irrespective of the value of the matching
		 * structure from the other set, then this property is required. Corresponds to
		 * the first row in the logic diagram being all zeros. For example, this is true
		 * for 'and' as well as 'andNot', and for 'clear', since false" & "x" = "false",
		 * and "false" &! "x" = "false".
		 */
		static final int F_OP_X_EQ_F = 0x2;

		/**
		 * If when level2 areas or level3 areas from the other set are null will require
		 * the matching area or block in this set to be set to null, irrespective of the
		 * current values in the matching structure from the this, then this property is
		 * required. Corresponds to the first column in the logic diagram being all
		 * zero. For example, this is true for 'and', since "x" & "false" = "false", as
		 * well as for 'clear'.
		 */
		static final int X_OP_F_EQ_F = 0x4;

		/**
		 * If when a level3 area from the other set is null will require the matching
		 * area or block in this set to be left as it is, then this property is
		 * required. Corresponds to the first column of the logic diagram being equal to
		 * the left hand operand column. For example, this is true for 'or', 'xor', and
		 * 'andNot', since for all of these "x" op "false" = "x".
		 */
		 ...
		 *
		 * @return the int containing the bits representing the properties of this
		 *         strategy
		 * @since 1.6
		 */
		...
		/**
		 * Instances of this class are to be serially reusable. To start a particular
		 * use, an instance is (re-)started by calling this method. It is passed the
		 * reference to the other bit set (usually to allow a check on whether it is
		 * null or not, so as to simplify the implementation of the <i>block</i>()
		 * method.
		 *
		...
		/**
		 * Deal with a scan that include a partial word within a level3 block. All that
		 * is required is that the result be stored (if needed) into the given a set
		 * block at the correct position, and that the operation only affect those bits
		 * selected by 1 bits in the mask.
		 *
		...
		/**
		 * Deals with a part of a block that consists of whole words, starting with the
		 * given first index, and ending with the word before the last index. For the
		 * words processed, the return value should indicate whether all those resulting
		 * words were zero, or not.
		 *
		 * @param base the base index of the block (to be used if needed)
	 ...
	 * <p>
	 * If level1 of the <i>a</i> set is longer than level1 of the bit set <i>b</i>,
	 * then the unmatched virtual "entries" of the <i>b</i> set (beyond the actual
	 * length of <i>b</i>) corresponding to these are all false, hence the result of
	 * the "and" operation will be to make all these entries in this set to become
	 * false--hence just remove them, and then scan only those entries that could
	 * match entries in the bit set<i>b</i>. This clearing of the remainder of the
	 * <i>a</i> set is accomplished by selecting both <i>F_OP_X_EQ_F</i> and
	 * <i>X_OP_F_EQ_F</i>.
	 *
	 * <pre>
	 * and| 0 1 0| 0 0 1| 0 1
	...
	// -----------------------------------------------------------------------------
	/**
	 * AndNot of two sets. Where the <i>a</i> set is zero, it remains zero (i.e.,
	 * without entries or with zero words). On the other hand, where the <i>b</i>
	 * set is zero, the <i>a</i> remains unchanged.
	 * <p>
	 * If level1 of the <i>a</i> set is longer than level1 of the bit set <i>b</i>,
	 * then the unmatched virtual "entries" of the <i>b</i> set (beyond the actual
	 * length of <i>b</i>) corresponding to these are all false, hence the result of
	...
	// -----------------------------------------------------------------------------
	/**
	 * Equals compares bits in the <i>a</i> set with those in the <i>b</i> set. None
	 * of the values in either set are changed, although the <i>a</i> set may have
	 * all zero level 3 blocks replaced by null references (and similarly at level
	 * 2).
	 *
	 * <pre>
	 * equals| 0 1 0| 0 - 1| - -
	...
	protected class EqualsStrategy extends AbstractStrategy {
		...
		protected void start(SparseBitSet b) {
			...
			result = true;
			/*
			 * Equals does not change the content of the set, hence hash need not be reset.
			 */
		}
	 ...
	 * common with the <i>b</i> set. During the scan of the <i>a</i> set blocks (and
	 * areas) that are all zero may be replaced with empty blocks and areas (null
	 * references), but the value of the set is not changed (which is why
	 * X_OP_F_EQ_F is not selected, since this would cause parts of the <i>a</i> set
	 * to be zero-ed out).
	 *
	 * <pre>
	 * intersect| 0 1 0| 0 0 1| 1 1
	 * 
	 * <pre>
	 */
	protected class IntersectsStrategy extends AbstractStrategy {
		...
		protected void start(SparseBitSet b) {
			...
			result = false;
			/*
			 * Intersect does not change the content of the set, hence hash need not be
			 * reset.
			 */
		}
	 ...
	 * zeros in corresponding places, a zero results. Whole blocks or areas that are
	 * or become zero are replaced by null arrays.
	 * <p>
	 * If level1 of the <i>a</i> set is longer than level1 of the bit set <i>b</i>,
	 * then the unmatched entries of the <i>a</i> set (beyond the actual length of
	 * <i>b</i>) corresponding to these remain unchanged. *
	 * 
	 * <pre>
	 * or| 0 1 0| 0 1 1| 1 1
	 * 
	...
	// -----------------------------------------------------------------------------
	/**
	 * Set creates entries everywhere within the range. Hence no empty level2 areas
	 * or level3 blocks are ignored, and no empty (all zero) blocks are returned.
	 *
	 * <pre>
	 * set| 0 1 0| 1 1 1| 1 1
	 * 
	...
	// -----------------------------------------------------------------------------
	/**
	 * Update the seven statistics that are computed for each set. These are updated
	 * by calling <i>statisticsUpdate</i>, which uses this strategy.
	 *
	 * <pre>
	 * update| 0 1 0| 0 0 1| 1 1
	 * 
	 * <pre>
	 *
	...
	protected class UpdateStrategy extends AbstractStrategy {
		/**
		 * Working space for find the size and length of the bit set. Holds the index of
		 * the first non-empty word in the set.
		 */
		protected transient int wMin;

		/**
		 * Working space for find the size and length of the bit set. Holds copy of the
		 * first non-empty word in the set.
		 */
		protected transient long wordMin;

		/**
		 * Working space for find the size and length of the bit set. Holds the index of
		 * the last non-empty word in the set.
		 */
		protected transient int wMax;

		/**
		 * Working space for find the size and length of the bit set. Holds a copy of
		 * the last non-empty word in the set.
		 */
		protected transient long wordMax;

		/**
		 * Working space for find the hash value of the bit set. Holds the current state
		 * of the computation of the hash value. This value is ultimately transferred to
		 * the Cache object.
		 *
		 * @see SparseBitSet.Cache
		 */
		...
		/**
		 * Working space for keeping count of the number of non-zero words in the bit
		 * set. Holds the current state of the computation of the count. This value is
		...
		/**
		 * Working space for counting the number of non-zero bits in the bit set. Holds
		 * the current state of the computation of the cardinality.This value is
		 * ultimately transferred to the Cache object.
		 *
		 * @see SparseBitSet.Cache
		 */
		...
		/**
		 * This method initializes the computations by suitably resetting cache fields
		 * or working fields.
		 *
		...
		/**
		 * This method does the accumulation of the statistics. It must be called in
		 * sequential order of the words in the set for which the statistics are being
		 * accumulated, and only for non-null values of the second parameter.
		 *
		 * Two of the values (a2Count and a3Count) are not updated here, but are done in
		 * the code near where this method is called.
		 *
		...
		private void compute(final int index, final long word) {
			...
			hash ^= word * (index + 1);
			/*
			 * The first non-zero word contains the first actual bit of the set. The
			 * location of this bit is used to compute the set size.
			 */
			if (wMin < 0) {
				wMin = index;
				wordMin = word;
			...
			/*
			 * The last non-zero word contains the last actual bit of the set. The location
			 * of this bit is used to compute the set length.
			 */
			wMax = index;
			wordMax = word;
