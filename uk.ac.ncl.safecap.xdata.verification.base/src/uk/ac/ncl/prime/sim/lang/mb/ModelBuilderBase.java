...
public abstract class ModelBuilderBase extends ModelSolver {

	protected boolean solvable, definate;
	protected Object[][] members;

	public ModelBuilderBase(TypingContext typingContext, CLExpression predicate, List<CLParameter> variables) {
	...
	public ModelBuilderBase(TypingContext typingContext, CLExpression predicate, Collection<String> variables) {
	...
	void buildIndices() throws CLExecutionException, InvalidSetOpException {
		...
		for (final CLParameter v : getVariables()) {
			...
			if (exactValue != null) {
				...
				final Object setValues = inferredSetValues(getPredicate(), name);
				if (setValues instanceof BSet) {
					final BSet set = (BSet) setValues;
					if (set.card() > 0) {
	...
	private Object inferredSetValues(CLExpression expr, String v) {
		try {
			if (expr instanceof CLMultiExpression) {
				...
				if (binary.getLeft() instanceof CLIdentifier) {
					final CLIdentifier id = (CLIdentifier) binary.getLeft();
					if (id.getName().equals(v)) {
						return binary.getRight().getValueInterpreted(getTypingContext().getRuntimeExecutionContext());
					}
				}
			}
	...
	private Object inferredExactValue(CLExpression expr, String v) throws CLExecutionException, InvalidSetOpException {
		try {
			if (expr instanceof CLMultiExpression) {
				...
				} else if (binary.getLeft() instanceof CLIdentifier) {
					final CLIdentifier id = (CLIdentifier) binary.getRight();
					if (id.getName().equals(v)) {
						return binary.getLeft().getValueInterpreted(getTypingContext().getRuntimeExecutionContext());
					}
				}
			}
		} catch (final Throwable e) {
