...
/**
 * Train head reaches station stopping point
 * 
 * @author alex
 *
 */
public class HeadToStation implements ITrainEventSource {
	private final SystemState state;

	public HeadToStation(SystemState state) {
		this.state = state;
	}
	...
	public TrainEvent getTrainEventFor(S1TrainActor train) {
		...
		EquipmentPosition limit = train.getLine().getNextStation(train.getHead());
		...
		if (limit.distance == 0) {
			limit = train.getLine().getNextStation(train.getHead() + Delta.DELTA);
			if (limit.getEquipment() == null) {
				return TrainEvent.UNLIMITED;
			}
		...
		event.setDistanceOracle("HeadToStation");
