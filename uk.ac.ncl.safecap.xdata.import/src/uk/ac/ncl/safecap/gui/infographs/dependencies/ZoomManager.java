...
public class ZoomManager {
	...
	private double getFitXZoomLevel(int which) {
		...
		final double scaleX = Math.min(available.width * zoom / desired.width, getMaxZoom());
		final double scaleY = Math.min(available.height * zoom / desired.height, getMaxZoom());
		if (which == 0) {
			return scaleX;
		}
		if (which == 1) {
	...
	public String getZoomAsText() {
		...
		final String newItem = format(zoom * multiplier);
		return newItem;
	}
	...
	public String[] getZoomLevelsAsText() {
		final String[] zoomLevelStrings = new String[zoomLevels.length + zoomLevelContributions.size()];
	...
	protected void primSetZoom(double zoom) {
		final Point p1 = getViewport().getClientArea().getCenter();
		final Point p2 = p1.getCopy();
		final Point p = getViewport().getViewLocation();
		final double prevZoom = this.zoom;
		this.zoom = zoom;
		pane.setScale(zoom);
		fireZoomChanged();
		...
		final Dimension dif = p2.getDifference(p1);
		p.x += dif.width;
		p.y += dif.height;
		setViewLocation(p);
	}
	...
	public void setZoomAsText(String zoomString) {
		...
		if (zoomString.equalsIgnoreCase(FIT_HEIGHT)) {
			...
			try {
				...
				final double newZoom = Double.parseDouble(zoomString) / 100;
