...
public interface ILineSeries extends ISeries {
	...
	/**
	 * Sets the symbol colors. Typically, the number of symbol colors is the same as
	 * the number of plots. If the number of symbol colors is less than the number
	 * of plots, the rest of plots will have the common color which is set with
	 * <tt>setSymbolColor(Color)</tt>.
	 * 
	 * <p>
	 * By default, <tt>null</tt> is set.
	 * 
	 * @param colors the symbol colors. If <tt>null</tt> is given, the color which
	...
	/**
	 * Sets the width of line connecting data points and also line drawing symbol if
	 * applicable (i.e. <tt>PlotSymbolType.CROSS</tt> or
	 ...
	 * 
	 * @param enabled true if enabling area chart
	 */
	 ...
	 * 
	 * @param enabled true if enabling step chart
	 */
	 ...
	 * <p>
	 * If number of data points is too large, the series is drawn as a collection of
	 * dots rather than lines. In this case, the anti-alias doesn't really make
	 * effect, and just causes performance degradation. Therefore, client code may
	 * automatically enable/disable the anti-alias for each series depending on the
	 * number of data points, or alternatively may let end-user configure it.
	 * 
	 * @param antialias the anti-aliasing value which can be <tt>SWT.DEFAULT</tt>,
	 *                  <tt>SWT.ON</tt> or <tt>SWT.OFF</tt>.
	 */
