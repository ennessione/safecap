...
public class CompressBarSeries extends Compress {
	...
	private boolean isInSameGridXAsPrevious(double x) {
		final int xGridIndex = (int) ((x - config.getXLowerValue()) / (config.getXUpperValue() - config.getXLowerValue())
				* config.getWidthInPixel());

		final boolean isInSameGridAsPrevious = xGridIndex == previousXGridIndex;
