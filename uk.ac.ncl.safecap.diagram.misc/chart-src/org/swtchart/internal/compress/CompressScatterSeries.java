...
public class CompressScatterSeries extends Compress {
	...
	protected void addNecessaryPlots(ArrayList<Double> xList, ArrayList<Double> yList, ArrayList<Integer> indexList) {
		...
		if (isLineVisible) {
			...
			final int width = (int) config.getWidthInPixel();
			final int height = (int) config.getHeightInPixel();
			...
			for (int i = 0; i < xSeries.length; i++) {
				if (xSeries[i] >= xLower && xSeries[i] <= xUpper && ySeries[i] >= yLower && ySeries[i] <= yUpper
	...
	private boolean isOccupied(double x, double y) {
		...
		if (config.isXLogScale()) {
			final double lower = Math.log10(config.getXLowerValue());
			final double upper = Math.log10(config.getXUpperValue());
			xGridIndex = (int) ((Math.log10(x) - lower) / (upper - lower) * config.getWidthInPixel());
		} else {
			xGridIndex = (int) ((x - config.getXLowerValue()) / (config.getXUpperValue() - config.getXLowerValue())
					* config.getWidthInPixel());
		}

		// calculate the Y grid index
		if (config.isYLogScale()) {
			final double lower = Math.log10(config.getYLowerValue());
			final double upper = Math.log10(config.getYUpperValue());
			yGridIndex = (int) ((Math.log10(y) - lower) / (upper - lower) * config.getHeightInPixel());
		} else {
			yGridIndex = (int) ((y - config.getYLowerValue()) / (config.getYUpperValue() - config.getYLowerValue())
					* config.getHeightInPixel());
		}

		final boolean isOccupied = occupied[xGridIndex][yGridIndex];
	 ...
	 * 
	 * @param visible the state indicating if the line is visible
	 */
