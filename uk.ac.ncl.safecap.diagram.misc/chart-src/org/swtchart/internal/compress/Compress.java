...
//        return copiedSeries;
	 ...
	 * 
	 * @param xList     the array in which x coordinate for necessary plot is stored
	 * @param yList     the array in which y coordinate for necessary plot is stored
	 * @param indexList the array in which series index for necessary plot is stored
	 */
	 ...
	 * 
	 * @param xList     the list to store the X coordinate
	 * @param yList     the list to store the Y coordinate
	 * @param indexList the list to store the series index
	 * @param x         the X coordinate
	 * @param y         the Y coordinate
	 * @param index     the series index
	 */
	protected void addToList(ArrayList<Double> xList, ArrayList<Double> yList, ArrayList<Integer> indexList, double x, double y,
			int index) {
		xList.add(x);
		yList.add(y);
		indexList.add(index);
	...
	protected boolean isInSameGridAsPrevious(double x, double y) {
		...
		if (isXLogScale) {
			final double lower = Math.log10(xLower);
			final double upper = Math.log10(xUpper);
		...
		if (isYLogScale) {
			final double lower = Math.log10(yLower);
			final double upper = Math.log10(yUpper);
		...
		final boolean isInSameGridAsPrevious = xGridIndex == previousXGridIndex && yGridIndex == previousYGridIndex;
