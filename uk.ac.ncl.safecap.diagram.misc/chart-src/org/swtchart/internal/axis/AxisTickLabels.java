...
public class AxisTickLabels implements PaintListener {
	...
	private void updateTickLabelForDateAxis(int length) {
		final double min = axis.getRange().lower;
		final double max = axis.getRange().upper;

		final double gridStepHint = Math.abs(max - min) / length * axis.getTick().getTickMarkStepHint();
		...
		if (timeUnit == Calendar.MILLISECOND || timeUnit == Calendar.SECOND || timeUnit == Calendar.MINUTE
			...
			final Integer[] steps = possibleTickSteps.get(timeUnit);
			for (int i = 0; i < steps.length - 1; i++) {
				if (gridStepHint < (getPeriodInMillis(timeUnit, steps[i]) + getPeriodInMillis(timeUnit, steps[i + 1])) / 2d) {
					final BigDecimal gridStep = new BigDecimal(Long.valueOf(getPeriodInMillis(timeUnit, steps[i])).toString());
	...
	private void updateTickLabelForMonthOrYear(int length, double gridStepHint, int tickStepUnit) {
		...
		// get tick step
		final Integer[] steps = possibleTickSteps.get(tickStepUnit);
		...
		while (cal.getTimeInMillis() < max) {
			...
			final int tickLabelPosition = (int) ((cal.getTimeInMillis() - min) / (max - min) * length);
	...
	private void updateTickLabelForCategoryAxis(int length) {
		final String[] series = axis.getCategorySeries();
		if (series == null) {
			return;
		}

		final int min = (int) axis.getRange().lower;
		final int max = (int) axis.getRange().upper;

		final int sizeOfTickLabels = series.length < max - min + 1 ? series.length : max - min + 1;
		final int initialIndex = min < 0 ? 0 : min;

		for (int i = 0; i < sizeOfTickLabels; i++) {
			tickLabels.add(series[i + initialIndex]);

			final int tickLabelPosition = (int) (length * (i + 0.5) / sizeOfTickLabels);
			tickLabelPositions.add(tickLabelPosition);
		}
	}
	...
	private void updateTickLabelForLogScale(int length) {
		...
		final int digitMin = (int) Math.ceil(Math.log10(min));
		final int digitMax = (int) Math.ceil(Math.log10(max));
		...
		for (int i = digitMin; i <= digitMax; i++) {
			for (BigDecimal j = firstPosition; j.doubleValue() <= pow(10, i).doubleValue(); j = j.add(tickStep)) {
				...
				if (axis.isDateEnabled()) {
					final Date date = new Date((long) j.doubleValue());
				...
				final int tickLabelPosition = (int) ((Math.log10(j.doubleValue()) - Math.log10(min)) / (Math.log10(max) - Math.log10(min))
						* length);
				tickLabelPositions.add(tickLabelPosition);
			}
	...
	private void updateTickLabelForLinearScale(int length, BigDecimal tickStep) {
		...
		if (axis.isDateEnabled()) {
			final BigDecimal zeroOclock = firstPosition.subtract(new BigDecimal(new Double(3600000).toString()));
		...
		for (BigDecimal b = firstPosition; b.doubleValue() <= max; b = b.add(tickStep)) {
			if (axis.isDateEnabled()) {
				final Date date = new Date((long) b.doubleValue());
				tickLabels.add(format(date));
			} else {
			...
			final int tickLabelPosition = (int) ((b.doubleValue() - min) / (max - min) * length);
			tickLabelPositions.add(tickLabelPosition);
		}
	}
	...
	private void updateTickVisibility() {
		...
		for (int i = 0; i < tickLabelPositions.size(); i++) {
			...
			if (!axis.isValidCategoryAxis()) {
				...
				final String currentLabel = tickLabels.get(i);
				try {
					final double value = Double.parseDouble(currentLabel);
	...
	private int getTimeUnit(double gridStepHint) {
		...
		for (final Integer unit : units) {
			final Integer[] steps = possibleTickSteps.get(unit);
	...
	/**
	 * Gets the period in milliseconds of given unit of date and amount. The period
	 * is calculated based on UTC of January 1, 1970.
	 *
	...
	private boolean hasSpaceToDraw(int previousPosition, int tickLabelPosition, String tickLabel) {
		final Point p = Util.getExtentInGC(axis.getTick().getFont(), tickLabel);
		final int interval = tickLabelPosition - previousPosition;
		final int textLength = axis.isHorizontalAxis() ? p.x : p.y;
	...
	private void updateTickLabelMaxLength() {
		...
		for (int i = 0; i < tickLabels.size(); i++) {
			if (tickVisibilities.size() > i && tickVisibilities.get(i) == true) {
				final Point p = Util.getExtentInGC(axis.getTick().getFont(), tickLabels.get(i));
				if (p.x > maxLength) {
					maxLength = p.x;
				}
			}
	...
	private BigDecimal getGridStep(int lengthInPixels, double min, double max) {
		...
		final double length = Math.abs(max - min);
		final double gridStepHint = length / lengthInPixels * axis.getTick().getTickMarkStepHint();
	...
	protected void updateLayoutData() {
		...
		if (!axis.getTick().isVisible()) {
			...
			if (axis.isHorizontalAxis()) {
				final double angle = axis.getTick().getTickLabelAngle();
				heightHint = Axis.MARGIN + (int) (tickLabelMaxLength * Math.sin(Math.toRadians(angle))
	...
	private void drawXTick(GC gc) {
		final int offset = axis.getTick().getAxisTickMarks().getBounds().x;

		// draw tick labels
		gc.setFont(axis.getTick().getFont());
		final int angle = axis.getTick().getTickLabelAngle();
		for (int i = 0; i < tickLabelPositions.size(); i++) {
			if (axis.isValidCategoryAxis() || tickVisibilities.get(i) == true) {
				final String text = tickLabels.get(i);
				final int textWidth = gc.textExtent(text).x;
				final int textHeight = gc.textExtent(text).y;
				if (angle == 0) {
					final int x = (int) (tickLabelPositions.get(i) - textWidth / 2d + offset);
				...
				if (axis.getPosition() == Position.Primary) {
					x = (float) (offset + bounds.x + tickLabelPositions.get(i) - textWidth * Math.cos(Math.toRadians(angle))
	...
	private void drawRotatedText(GC gc, String text, float x, float y, int angle) {
		...
		final Image image = new Image(Display.getCurrent(), textWidth, textHeight);
	...
	private void drawYTick(GC gc) {
		final int margin = Axis.MARGIN + AxisTickMarks.TICK_LENGTH;

		// draw tick labels
		gc.setFont(axis.getTick().getFont());
		final int figureHeight = gc.textExtent("dummy").y;
		for (int i = 0; i < tickLabelPositions.size(); i++) {
			...
			if (tickVisibilities.get(i) == true) {
				...
				final int y = (int) (bounds.height - 1 - tickLabelPositions.get(i) - figureHeight / 2.0 - margin);
