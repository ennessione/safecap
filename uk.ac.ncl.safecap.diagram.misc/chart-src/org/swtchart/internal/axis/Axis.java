...
public class Axis implements IAxis {
	 ...
	 * @param update true if updating the chart layout
	 */
	public void setRange(Range range, boolean update) {
		...
		if (Double.isNaN(range.lower) || Double.isNaN(range.upper) || Double.isInfinite(range.lower) || Double.isInfinite(range.upper)
	...
	public void enableLogScale(boolean enabled) throws IllegalStateException {
		...
		if (enabled) {
			if (chart.getSeriesSet().getSeries().length == 0) {
				...
				final double minSeriesValue = getMinSeriesValue();
	 ...
	 * 
	 * @param update true if updating chart layout
	 */
	public void adjustRange(boolean update) {
		...
		for (final ISeries series : chart.getSeriesSet().getSeries()) {
			final int axisId = direction == Direction.X ? series.getXAxisId() : series.getYAxisId();
			...
			final Range range = ((Series) series).getAdjustedRange(this, length);
		...
		if (!Double.isNaN(minimum) && !Double.isNaN(maximum)) {
			if (minimum == maximum) {
				final double margin = minimum == 0 ? 1d : Math.abs(minimum / 2d);
				minimum -= margin;
				maximum += margin;
			}
	...
	public void zoomIn(double coordinate) {
		...
		if (isValidCategoryAxis()) {
			...
			final double digitMin = Math.log10(min);
			final double digitMax = Math.log10(max);
			final double digitCoordinate = Math.log10(coordinate);
	...
	public void scrollUp() {
		...
		if (isValidCategoryAxis()) {
			...
			final double digitMax = Math.log10(upper);
			final double digitMin = Math.log10(lower);
	...
	public void setCategorySeries(String[] series) {
		...
		final String[] copiedSeries = new String[series.length];
	 ...
	 * 
	 * @param dataCoordinate the data coordinate
	 * @param lower          the min value of range
	 * @param upper          the max value of range
	...
	public int getPixelCoordinate(double dataCoordinate, double lower, double upper) {
		...
		if (isHorizontalAxis) {
			if (logScaleEnabled) {
				pixelCoordinate = (int) ((Math.log10(dataCoordinate) - Math.log10(lower)) / (Math.log10(upper) - Math.log10(lower))
						* width);
			...
			if (logScaleEnabled) {
				pixelCoordinate = (int) ((Math.log10(upper) - Math.log10(dataCoordinate)) / (Math.log10(upper) - Math.log10(lower))
						* height);
			} else if (categoryAxisEnabled) {
	...
	/**
	 * Gets the data coordinate corresponding to the given pixel coordinate on plot
	 * area.
	 * 
	 * @param pixelCoordinate the pixel coordinate on plot area
	 * @param lower           the min value of range
	 * @param upper           the max value of range
	 ...
	 * 
	 * @param numRisers the number of risers per category
	 */
	...
	/**
	 * Checks if the axis is horizontal. X axis is not always horizontal. Y axis can
	 * be horizontal with <tt>Chart.setOrientation(SWT.VERTICAL)</tt>.
	 * 
	 * @return true if the axis is horizontal
	 */
	public boolean isHorizontalAxis() {
		final int orientation = chart.getOrientation();
	...
	protected void dispose() {
		...
		for (final IDisposeListener listener : listeners) {
