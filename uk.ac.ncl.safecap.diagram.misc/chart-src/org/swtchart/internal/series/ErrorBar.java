...
public class ErrorBar implements IErrorBar {
	...
	public double[] getPlusErrors() {
		...
		final double[] copiedSeries = new double[plusErrors.length];
	...
	public void setPlusErrors(double[] errors) {
		...
		plusErrors = new double[errors.length];
	...
	public double[] getMinusErrors() {
		...
		final double[] copiedSeries = new double[minusErrors.length];
	...
	public void setMinusErrors(double[] errors) {
		...
		minusErrors = new double[errors.length];
	 ...
	 * @param h           the horizontal coordinate to draw error bar
	 * @param v           the vertical coordinate to draw error bar
	 ...
	 * @param v          the vertical coordinate to draw error bar
	 * @param axis       the axis
	 * @param plusError  the plus error
	 * @param minusError the minus error
	 */
	private void draw(GC gc, int h, int v, Axis axis, double plusError, double minusError) {
		if (axis.isHorizontalAxis()) {
			final double dataCoordinate = axis.getDataCoordinate(h);
			final int plusErrorInPixels = axis.getPixelCoordinate(dataCoordinate + plusError) - h;
			...
			final double dataCoordinate = axis.getDataCoordinate(v);
			final int plusErrorInPixels = v - axis.getPixelCoordinate(dataCoordinate + plusError);
