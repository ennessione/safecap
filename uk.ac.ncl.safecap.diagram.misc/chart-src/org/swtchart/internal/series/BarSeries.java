...
public class BarSeries extends Series implements IBarSeries {
	...
	public Rectangle[] getBounds() {
		final Rectangle[] compressedBounds = getBoundsForCompressedSeries();
		if (((Axis) chart.getAxisSet().getXAxis(xAxisId)).isValidCategoryAxis()) {
			return compressedBounds;
		}

		final Rectangle[] rs = new Rectangle[xSeries.length];
		final double[] comporessedXSeries = compressor.getCompressedXSeries();
		int cnt = 0;
	...
	private Rectangle[] getBoundsForCompressedSeries() {
		final Axis xAxis = (Axis) chart.getAxisSet().getXAxis(xAxisId);
		final Axis yAxis = (Axis) chart.getAxisSet().getYAxis(yAxisId);
		...
		final Rectangle[] rectangles = new Rectangle[xseries.length];
		...
		for (int i = 0; i < xseries.length; i++) {
			...
			final double riserHeight = Math.abs(yAxis.getPixelCoordinate(yseries[i], yRange.lower, yRange.upper)
					- yAxis.getPixelCoordinate(yAxis.isLogScaleEnabled() ? yRange.lower : baseYCoordinate, yRange.lower, yRange.upper));

			// adjust riser x coordinate and riser width for multiple series
			final int riserCnt = xAxis.getNumRisers();
			if (riserCnt > 1) {
			...
			if (xAxis.isHorizontalAxis()) {
				...
				rectangles[i] = getVisibleRectangle((int) (y - riserHeight), (int) Math.floor(x - riserwidth / 2d), (int) riserHeight,
						height);
			}
		}
	...
	private Rectangle getVisibleRectangle(int x, int y, int width, int height) {
		...
		final Point size = chart.getPlotArea().getSize();
		if (x + width > size.x) {
	 ...
	 * 
	 * @param riserIndex the index of riser in a category
	 */
	...
	public Range getAdjustedRange(Axis axis, int length) {
		...
		if (axis.getDirection() == Direction.X) {
			final double lowerRiserWidth = getRiserWidth(xSeries, 0, axis, minX, maxX);
			final double upperRiserWidth = getRiserWidth(xSeries, xSeries.length - 1, axis, minX, maxX);
	...
	/**
	 * Gets the color for riser frame. The color will be darker or lighter than the
	 * given color.
	 * 
	 * @param color the riser color
	...
	protected void draw(GC gc, int width, int height, Axis xAxis, Axis yAxis) {
		...
		// draw riser
		final Rectangle[] rs = getBoundsForCompressedSeries();
	 ...
	 * @param width  the riser width
	 * @param height the riser height
	 */
	private void drawRiser(GC gc, int h, int v, int width, int height) {
		...
		final Color frameColor = getFrameColor(getBarColor());
