...
//        System.arraycopy(series, 0, xSeries, 0, series.length);
		...
		if (minX <= 0) {
			final IAxis axis = chart.getAxisSet().getXAxis(xAxisId);
			if (axis != null) {
...
//        System.arraycopy(series, 0, ySeries, 0, series.length);
		...
		if (xSeries == null || xSeries.length != ySeries.length) {
		...
		if (minY <= 0) {
			final IAxis axis = chart.getAxisSet().getYAxis(yAxisId);
			if (axis != null) {
				axis.enableLogScale(false);
			}
			stackEnabled = false;
...
//        return copiedSeries;
	...
	public Date[] getXDateSeries() {
		...
		final Date[] series = new Date[xSeries.length];
		for (int i = 0; i < series.length; i++) {
	...
	/**
	 * Gets the adjusted range to show all series in screen. This range includes the
	 * size of plot like symbol or bar.
	 * 
	 * @param axis   the axis
	 ...
	 * 
	 * @param lowerPlotMargin the lower margin in pixels
	 * @param upperPlotMargin the upper margin in pixels
	 * @param length          the axis length in pixels
	 * @param axis            the axis
	...
	protected Range getRangeWithMargin(int lowerPlotMargin, int upperPlotMargin, int length, Axis axis, Range range) {
		...
		final int lowerPixelCoordinate = axis.getPixelCoordinate(range.lower, range.lower, range.upper)
				+ lowerPlotMargin * (axis.isHorizontalAxis() ? -1 : 1);
		final int upperPixelCoordinate = axis.getPixelCoordinate(range.upper, range.lower, range.upper)
	...
	public void draw(GC gc, int width, int height) {
		...
		final Axis xAxis = (Axis) chart.getAxisSet().getXAxis(getXAxisId());
		final Axis yAxis = (Axis) chart.getAxisSet().getYAxis(getYAxisId());
