...
public class Legend extends Composite implements ILegend, PaintListener {
	...
	private final Map<String, Rectangle> cellBounds;
	...
	/**
	 * Sorts the given series array. For instance, if there are two stack series in
	 * horizontal orientation, the top of stack series should appear at top of
	 * legend.
	 * <p>
	 * If there are multiple x axes, the given series array will be sorted with x
	 * axis first. And then, the series in each x axis will be sorted with
	...
	private ISeries[] sort(ISeries[] seriesArray) {
		...
		final Map<Integer, List<ISeries>> map = new HashMap<>();
		for (final ISeries series : seriesArray) {
			final int axisId = series.getXAxisId();
		...
		final List<ISeries> sortedArray = new ArrayList<>();
		final boolean isVertical = chart.getOrientation() == SWT.VERTICAL;
		for (final Entry<Integer, List<ISeries>> entry : map.entrySet()) {
			final boolean isCategoryEnabled = chart.getAxisSet().getXAxis(entry.getKey()).isCategoryEnabled();
	 ...
	 * <ul>
	 * <li>The stacked series will be gathered, and the order of stack series will
	 * be reversed.</li>
	 * <li>In the case of vertical orientation, the order of whole series will be
	 * reversed.</li>
	 * </ul>
	 * 
	...
	public void updateLayoutData() {
		...
		final ISeries[] seriesArray = sort(chart.getSeriesSet().getSeries());

		final Rectangle r = chart.getClientArea();
		final int titleHeight = ((Composite) chart.getTitle()).getSize().y;
		final int cellHeight = Util.getExtentInGC(getFont(), "dummy").y;
		...
		if (position == SWT.RIGHT || position == SWT.LEFT) {
			...
			for (final ISeries series : seriesArray) {
				...
				final int textWidth = Util.getExtentInGC(getFont(), series.getId()).x;
				final int cellWidth = textWidth + SYMBOL_WIDTH + MARGIN * 3;
				...
				cellBounds.put(series.getId(),
						new Rectangle(maxCellWidth * (columns - 1), yPosition - cellHeight - MARGIN, cellWidth, cellHeight));
			...
			for (final ISeries series : seriesArray) {
				...
				cellBounds.put(series.getId(),
						new Rectangle(xPosition - cellWidth, (cellHeight + MARGIN) * (rows - 1) + MARGIN, cellWidth, cellHeight));
	...
	protected void drawSymbol(GC gc, Series series, Rectangle r) {
		...
		if (series instanceof ILineSeries) {
			...
			final int lineStyle = Util.getIndexDefinedInSWT(((ILineSeries) series).getLineStyle());
			final int x = r.x;
			...
			final Color[] colors = ((ILineSeries) series).getSymbolColors();
			...
			final int size = SYMBOL_WIDTH / 2;
			final int x = r.x + size / 2;
			final int y = (int) (r.y - size / 2d + r.height / 2d);
	...
	public void paintControl(PaintEvent e) {
		...
		final GC gc = e.gc;
		gc.setFont(getFont());
		final ISeries[] seriesArray = chart.getSeriesSet().getSeries();
		...
		for (int i = 0; i < seriesArray.length; i++) {
			...
			final String id = seriesArray[i].getId();
			final Rectangle r = cellBounds.get(id);
