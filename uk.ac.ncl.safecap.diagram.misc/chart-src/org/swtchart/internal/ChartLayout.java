...
public class ChartLayout extends Layout {
	...
	private boolean parseControls(Composite composite) {
		...
		if (composite instanceof Chart) {
			final IAxisSet axisSet = ((Chart) composite).getAxisSet();
			if (axisSet != null) {
				final IAxis[] axisArray = axisSet.getAxes();
				for (final IAxis axis : axisArray) {
					axes.add((Axis) axis);
				}
			}
		}
	...
	private void computeControlSize() {
		...
		for (final Axis axis : axes) {
			final AxisLayoutData layoutData = new AxisLayoutData(axis);
	...
	private void layoutTitle(Rectangle r) {
		final int x = (int) ((r.width - titleWidth) / 2d);
		final int y = MARGIN;
		final int width = titleWidth;
		final int height = titleHeight;
	...
	private void layoutLegend(Rectangle r) {
		final int legendPosition = legend.getPosition();

		final int tHeight = titleHeight + (titleHeight == 0 ? 0 : PADDING);
		int x;
		int y;
		...
		final int width = legendWidth;
		final int height = legendHeight;
	...
	private void layoutPlot(Rectangle r) {
		...
		final int x = leftAxisWidth + MARGIN + (legendPosition == SWT.LEFT ? legendWidth + (legendWidth == 0 ? 0 : PADDING) : 0);
		final int y = titleHeight + topAxisHeight + MARGIN + (titleHeight == 0 ? 0 : PADDING)
				+ (legendPosition == SWT.TOP ? legendHeight + (legendHeight == 0 ? 0 : PADDING) : 0);
		final int width = r.width - leftAxisWidth - rightAxisWidth
				- (legendPosition == SWT.LEFT || legendPosition == SWT.RIGHT ? legendWidth + (legendWidth == 0 ? 0 : PADDING) : 0)
				- MARGIN * 2;
		final int height = r.height - bottomAxisHeight - topAxisHeight - titleHeight - MARGIN * 2 - (titleHeight == 0 ? 0 : PADDING)
	...
	private void layoutLeftAxis(Rectangle r, AxisLayoutData layoutData) {
		...
		final int yAxisMargin = Axis.MARGIN + AxisTickMarks.TICK_LENGTH;
