...
public class Grid implements IGrid {
	 ...
	 * @param width  the width to draw grid
	 * @param height the height to draw grid
	 */
	protected void draw(GC gc, int width, int height) {
		...
		final ArrayList<Integer> tickLabelPosition = axis.getTick().getAxisTickLabels().getTickLabelPositions();
		...
		if (axis.isValidCategoryAxis()) {
			...
			for (int i = 0; i < tickLabelPosition.size(); i++) {
				final int x = tickLabelPosition.get(i).intValue();
				if (x >= xWidth) {
					continue;
				}
