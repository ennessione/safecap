 ...
 * @generated
 */
public class SafecapValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = SafecapDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof SafecapDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && ProjectEditPart.MODEL_ID.equals(SafecapVisualIDRegistry.getModelID(view));
	}
	...
	private static void refreshDecorators(String viewId, Diagram diagram) {
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					if (TransactionUtil.getEditingDomain(fdiagram) != null) {
						TransactionUtil.getEditingDomain(fdiagram).runExclusive(new Runnable() {

							@Override
							public void run() {
								for (final Iterator it = decorators.iterator(); it.hasNext();) {
									final IDecorator decorator = (IDecorator) it.next();
									decorator.refresh();
								}
							}
						});
					...
					SafecapDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				TransactionUtil.getEditingDomain(view).runExclusive(new Runnable() {
					...
					@Override
					public void run() {
						viewId = view != null ? ViewUtil.getIdStr(view) : null;
					}
				});
			} catch (final Exception e) {
				SafecapDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$
			}
		}
		...
		public void refresh() {
			...
			final String elementId = ViewUtil.getIdStr(view);
			if (elementId == null) {
				return;
			}
			int severity = IMarker.SEVERITY_INFO;
			IMarker foundMarker = null;
			final IResource resource = WorkspaceSynchronizer.getFile(view.eResource());
			...
			try {
				...
				SafecapDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
			...
			for (int i = 0; i < markers.length; i++) {
				...
				final String attribute = marker.getAttribute(org.eclipse.gmf.runtime.common.core.resources.IMarker.ELEMENT_ID, ""); //$NON-NLS-1$
				if (attribute.equals(elementId)) {
					final int nextSeverity = marker.getAttribute(IMarker.SEVERITY, IMarker.SEVERITY_INFO);
					final Image nextImage = getImage(nextSeverity);
					if (foundMarker == null) {
						foundMarker = marker;
						...
						if (toolTip.getChildren().isEmpty()) {
							final Label comositeLabel = new Label();
							final FlowLayout fl = new FlowLayout(false);
			...
			if (editPart instanceof org.eclipse.gef.GraphicalEditPart) {
				if (view instanceof Edge) {
					...
					setDecoration(getDecoratorTarget().addShapeDecoration(getImage(severity), IDecoratorTarget.Direction.NORTH_EAST, margin,
							true));
				}
		...
		public void activate() {
			...
			final Diagram diagramView = view.getDiagram();
			if (diagramView == null) {
				return;
			}
		...
		public void deactivate() {
			...
			final List list = (List) allDecorators.get(viewId);
			if (list != null) {
				list.remove(this);
				if (list.isEmpty()) {
	...
	static class MarkerObserver implements IFileObserver {
		...
		public void handleMarkerDeleted(IMarker marker, Map attributes) {
			final String viewId = (String) attributes.get(org.eclipse.gmf.runtime.common.core.resources.IMarker.ELEMENT_ID);
		...
		public void handleMarkerChanged(IMarker marker) {
			...
			final String viewId = marker.getAttribute(org.eclipse.gmf.runtime.common.core.resources.IMarker.ELEMENT_ID, ""); //$NON-NLS-1$
			refreshDecorators(viewId, diagram);
		}
		...
		private String getType(IMarker marker) {
			try {
				...
				SafecapDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
